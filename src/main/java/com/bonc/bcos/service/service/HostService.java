...
//	 * 校验三、有任务在执行，集群状态主机状态都无法变更		@ 主机锁控制之后可以将执行校验改为主机锁校验
	 ...
	 */
	void deleteHost(String ip,String code) ;
	 ...
	 * }]
	 */
	List<SysClusterHost> findHosts(String code);

	/**
	 *  根据模板文件上传主机信息
	 * @param template 模板文件
	 */
    void saveTemplate(InputStream template,String code) throws IOException;

	void handleData(String code);
