...
//	 * 	2. 设备表里有数据，如果是使用中:状态无法进行更改;如果不是:更新完成之后,清空    该主机    对应的设备表的所有角色记录(未使用的状态),并修改角色状态为未分配
	 ...
	 * 
	 */
	void saveRoles(List<SysClusterHost> hosts,String code);
	 ...
	 * @param global  全局配置
	 */
	void saveGlobal(Map<String,String> global,String code) ;

	/**
	 * 查询全局配置
	 * @return map 全局配置
	 */
	List<SysClusterInfo> findGlobal(String code);

    List<SysClusterStoreCfg> storeCfg();

    List<SysClusterRole> roleCfg();

	void unlockHost(SysClusterHost host,String code);

	List<SysClusterHost> rolePolicy(List<SysClusterHost> hosts,String code);

	List<SysClusterHost> rolePolicy(String code);

	List<SysClusterVersionImage> findImage(String version);

    List<SysClusterHostRole> getRoles(String ip, String code);
}
