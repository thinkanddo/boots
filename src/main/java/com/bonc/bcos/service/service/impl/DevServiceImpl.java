...
import com.bonc.bcos.service.repository.SysClusterStoreCfgRepository;
...
public class DevServiceImpl implements DevService {
    ...
    public DevServiceImpl(SysClusterHostRoleRepository clusterHostRoleDao, SysClusterHostRoleDevRepository clusterHostRoleDevDao,
                          SysClusterStoreCfgRepository clusterStoreCfgDao, ClusterService clusterService) {
    ...
    @Override
    public List<SysClusterHostRoleDev> findDev(String code,String ip,String roleId) {
        // 向上兼容不传输
        if (StringUtils.isEmpty(roleId)){
            SysClusterHostRole hostRole = clusterHostRoleDao.findByCodeAndIpAndRoleCode(code,ip, SysClusterRole.DEFAULT_ROLE);

            if (null==hostRole){
                return new ArrayList<>();
            }

            roleId = hostRole.getId();
        }
        return clusterHostRoleDevDao.findByHostRoleIdOrderByDevName(roleId);
    }

    @Override
    @Transactional
    public void changeDevStatus(SysClusterHostRoleDev changeDev)  {
        // 检查设备是否存在
        Optional<SysClusterHostRoleDev> optional = clusterHostRoleDevDao.findById(changeDev.getId());
        ...
        // 通过比较之前设备状态和当前期望的设备状态，判断是否更新状态
        char devStatus = dev.getStatus();
        char devChangeStatus = changeDev.getStatus();


        if (devStatus == SysClusterHostRoleDev.UN_USED && devChangeStatus == SysClusterHostRoleDev.UN_FORMAT){
            dev.unformat();
        }else if ((devChangeStatus == SysClusterHostRoleDev.UN_USED && devStatus == SysClusterHostRoleDev.UN_FORMAT) || (devChangeStatus == SysClusterHostRoleDev.UN_USED && devStatus == SysClusterHostRoleDev.DISABLED)){
            dev.enable();
        }else if (devStatus == SysClusterHostRoleDev.UN_USED && devChangeStatus == SysClusterHostRoleDev.DISABLED){
            dev.disable();
        }

        if (devStatus != devChangeStatus){
            //  修改设备的状态
    ...
    class StoreAllocation{
        ...
        StoreAllocation(SysClusterHost host){
            ...
            for(SysClusterHostRole hostRole : host.getRoles().values()){
                if (!hostRole.isInstalled()&&!SysClusterRole.DEFAULT_ROLE.equals(hostRole.getRoleCode())){
                    ...
                    this.consumeCfg.addAll(getStoreCfg(roleCode));
            ...
            for(SysClusterHostRoleDev provide:this.provides.values()){
                allSize = allSize+provide.getEnableSize();
            }
            ...
            public void add(SysClusterStoreCfg costumer) {
                switch (costumer.getStoreType()){
                    ...
                    case SysClusterStoreCfg.TYPE_VG:
                        ...
                        break;
                    case SysClusterStoreCfg.TYPE_LVM:
                        vgLoads.add(costumer);
                        break;
                    case SysClusterStoreCfg.TYPE_MOUNT:
                        vgLoads.add(costumer);
                        break;
                }
            }
        ...
        private boolean allocateDev(AllocPlan optimalPlan) {
            ...
            for (SysClusterHostRoleDev dev: optimalPlan.planRatio.devRatio.keySet()){
                ...
                if (partLoads.size()>0){
                    ...
                    for (SysClusterStoreCfg consume: partLoads){
                        int ratioNum = realRatio*(floatRatio*(consume.getMaxSize()-consume.getMinSize())/100+consume.getMinSize())/100;
                        int size = dev.accessAlloc(ratioNum);
            ...
            for (SysClusterHostRoleDev dev: optimalPlan.planRatio.devRatio.keySet()){
                ...
                if (vgLoads.size()>0){
                    int realRatio = dev.getEnableSize()*100/(ratio.totalRatio-ratio.partRatio);
                    realRatio = realRatio>100?100:realRatio;
                    for (SysClusterStoreCfg consume: vgLoads){
                        int ratioNum = floatRatio*(consume.getMaxSize()-consume.getMinSize())/100+consume.getMinSize();
                        int realNum = realRatio*ratioNum/100;
                        if (dev.getEnableSize()<consume.getMinSize()){
                            remainConsumer.put(consume,ratioNum);
                        }else if (dev.getEnableSize()<ratioNum){
                            int size = dev.accessAlloc(dev.getEnableSize());
                            this.consumes.add(new SysClusterHostRoleDev(roleMap.get(consume.getRoleCode()),dev.getDevName(),dev.getPartType(),size,consume.getName()));
                        }else{
                            int size = dev.accessAlloc(realNum);
                            this.consumes.add(new SysClusterHostRoleDev(roleMap.get(consume.getRoleCode()),dev.getDevName(),dev.getPartType(),size,consume.getName()));
                        }
                    }
                }
        ...
        private boolean fullConsumer(HashMap<SysClusterStoreCfg,Integer> remainConsumer){
            ...
            for(SysClusterStoreCfg consume: remainConsumer.keySet()){
                if(consume.isPartType()) {continue;}
        ...
        AllocPlan finalPlan(List<AllocPlan> optimalPlans) {
            ...
            double varianceMin = Double.MAX_VALUE;
            ...
            for(AllocPlan plan:optimalPlans) {
                ...
                float planLoad = 0f;
                for (Map.Entry<SysClusterHostRoleDev,DevRatio> entry: planRatio.devRatio.entrySet()){
                    SysClusterHostRoleDev dev = entry.getKey();
                    DevRatio devRatio = entry.getValue();
                    devRatio.load = devRatio.totalRatio*1f/dev.getEnableSize();
                    planLoad+=devRatio.load;
                }
                float averageLoad = planLoad/provides.size();

                for(SysClusterHostRoleDev provider: provides.values()){
                    planRatio.varianceLoad += Math.pow(planRatio.contains(provider)?(planRatio.devRatio.get(provider).load-averageLoad):averageLoad,2);
                }

                if(planRatio.varianceLoad<varianceMin){
                    finalPlan = plan;
                    varianceMin = planRatio.varianceLoad;
                }
            }
            return finalPlan;
        }
        ...
        private void optimalPlans(List<AllocPlan> allocPlans) {
            ...
            for (AllocPlan plan: allocPlans){
                int planPartRatio = 0;
                int levelNum = 0;
                for (Map.Entry<SysClusterHostRoleDev,DevRatio> entry:plan.planRatio.devRatio.entrySet()){
                    SysClusterHostRoleDev dev = entry.getKey();
                    DevRatio ratio = entry.getValue();
                ...
                Integer maxNum = planLevelNum.get(planPartRatio);
                if (maxNum==null||maxNum<levelNum){
                    planLevelNum.put(planPartRatio,levelNum);
                }
        ...
        private  void calculatePlanRatio(List<AllocPlan> allocPlan) {
            ...
            while (iterator.hasNext()){
                ...
                for(Map.Entry<SysClusterStoreCfg, SysClusterHostRoleDev> entry:plan.entrySet()) {
                    ...
                    if(costumer.isPartType()){
                        ratio.partMin=ratio.partMin+costumer.getMinSize();
                        ratio.partRatio = ratio.partRatio+(floatRatio*(costumer.getMaxSize()-costumer.getMinSize())/100+costumer.getMinSize());
                    }
                    ratio.totalMin = ratio.totalMin+costumer.getMinSize();
                    ratio.totalRatio = ratio.totalRatio+(floatRatio*(costumer.getMaxSize()-costumer.getMinSize())/100+costumer.getMinSize());
    ...
    @Override
    @Transactional
    public void allocate(Set<String> targets,String code) {
        // 重新生成默认角色的分布
        List<SysClusterHost> hosts = clusterService.rolePolicy(code);
        ...
        if(null==targets||targets.isEmpty()){
            ...
            for(SysClusterHost host:hosts){
                if(targets.contains(host.getId().getIp()) && checkHost(host,errorHost)){
                    hostMap.put(host.getId().getIp(),host);
                }
                allHostIp.add(host.getId().getIp());
            }
            // targets的主机都必须存
        ...
        for (String ip : ips){
            ...
            if (flag){
                for(SysClusterHostRole role:sa.getRoleSet()){
                    if (!SysClusterRole.DEFAULT_ROLE.equals(role.getRoleCode())){
                        role.setStatus(SysClusterHostRole.PRE_HANDLE);
                    }
                }
            }
            saList.add(sa);
        ...
        if (errorHost.isEmpty()){
            for(StoreAllocation sa: saList){
                // 清理主机下所有角色没用的磁盘，重新生成
                for (SysClusterHostRole hostRole:sa.getRoleSet()){
                    clusterHostRoleDevDao.deleteByHostRoleIdAndStatus(hostRole.getId(),SysClusterHostRoleDev.UN_USED);
                }
                // 保存新生成的设备方案
                clusterHostRoleDevDao.saveAll(sa.getConsumes());
    ...
    @Override
    public List<SysClusterStoreCfg> getStoreCfg(String roleCode) {
        ...
        return storeCache.get(roleCode);
    }

    @Override
    public SysClusterHostRoleDev saveDev(String code, SysClusterHostRoleDev dev) {
        dev.setCode(code);
        return clusterHostRoleDevDao.save(dev);
    }

    @Override
    public boolean delDev(String code, SysClusterHostRoleDev dev) {
        Optional opt = clusterHostRoleDevDao.findById(dev.getId());
        if (opt.isPresent()){
            SysClusterHostRoleDev devOpt = (SysClusterHostRoleDev) opt.get();
            if (devOpt.getStatus() == SysClusterHostRoleDev.UN_USED){
                clusterHostRoleDevDao.deleteById(dev.getId());
                return true;
            }
        }
        return false;
